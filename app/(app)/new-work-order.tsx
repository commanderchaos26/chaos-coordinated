...
export default function NewWorkOrderScreen() {
  ...
  const saveAttempt = useRef<{ fingerprint: string; idempotencyKey: string } | null>(null);
  ...
  const submit = async () => {
    ...
    if (dueAt) { const parsed = new Date(dueAt); if (Number.isNaN(parsed.getTime())) { Alert.alert('Invalid due date','Choose the due date and time again.'); return; } dueIso=parsed.toISOString(); }
    ...
    try {
      ...
      Alert.alert(
        ...
        result.idempotent ? 'The earlier save reached the server. No duplicate was created.' : 'The work order is now available for dispatch.',
        [{text:'OK',onPress:()=>router.replace('/(app)/work-orders' as never)}],
      );
...
function Field({label,value,onChangeText,placeholder,multiline,keyboardType,maxLength}:{label:string;value:string;onChangeText:(value:string)=>void;placeholder:string;multiline?:boolean;keyboardType?:'default'|'numeric';maxLength?:number}){return <View style={styles.field}><Label text={label}/><TextInput value={value} onChangeText={onChangeText} placeholder={placeholder} placeholderTextColor={colors.subtle} multiline={multiline} keyboardType={keyboardType} maxLength={maxLength} textAlignVertical={multiline?'top':'center'} style={[styles.input,multiline&&styles.multiline]}/></View>}

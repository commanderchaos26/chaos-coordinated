...
export default function WorkOrdersScreen() {
  ...
  return <ScrollView contentContainerStyle={styles.root} showsVerticalScrollIndicator={false}>
    ...
    {canCreate(membership) ? <Pressable onPress={() => router.push('/(app)/new-work-order' as never)} style={styles.create}><Icon name="add" color={colors.background} size={20}/><Text style={styles.createText}>New work order</Text></Pressable> : null}
    ...
    </Card></Pressable>) : <EmptyState icon="construct-outline" title="Queue is clear" message={canCreate(membership) ? 'Create the first work order to start the field queue.' : 'New work orders will appear here when assigned.'} />}
  </ScrollView>;
}

const styles = StyleSheet.create({ root: { backgroundColor: colors.background, flexGrow: 1, padding: spacing.lg, paddingBottom: 110 }, header: { alignItems: 'center', flexDirection: 'row', justifyContent: 'space-between', marginBottom: spacing.lg, paddingTop: spacing.sm }, eyebrow: { color: colors.teal, fontSize: 11, fontWeight: '800', letterSpacing: 1.2 }, heading: { color: colors.text, ...typography.title, marginTop: 4 }, count: { alignItems: 'flex-end' }, countValue: { color: colors.text, fontSize: 26, fontWeight: '800' }, countLabel: { color: colors.subtle, fontSize: 10, fontWeight: '800', letterSpacing: 1 }, create: { alignItems: 'center', backgroundColor: colors.teal, borderRadius: 14, flexDirection: 'row', gap: spacing.sm, justifyContent: 'center', minHeight: 54, marginBottom: spacing.md }, createText: { color: colors.background, fontSize: 15, fontWeight: '900' }, search: { alignItems: 'center', backgroundColor: colors.surface, borderColor: colors.border, borderRadius: 14, borderWidth: 1, flexDirection: 'row', minHeight: 52, paddingHorizontal: spacing.md }, searchInput: { color: colors.text, flex: 1, fontSize: 15, marginLeft: spacing.sm }, orderCard: { marginBottom: spacing.sm }, card: { padding: spacing.md }, orderTop: { alignItems: 'center', flexDirection: 'row', justifyContent: 'space-between' }, status: { color: colors.muted, fontSize: 12, fontWeight: '700', textTransform: 'capitalize' }, orderTitle: { color: colors.text, fontSize: 17, fontWeight: '800', marginTop: spacing.md }, location: { color: colors.text, fontSize: 13, marginTop: spacing.sm }, department: { color: colors.teal, fontSize: 12, fontWeight: '800', marginTop: 5 }, metaLine: { alignItems: 'center', flexDirection: 'row', marginTop: spacing.md }, metaText: { color: colors.muted, fontSize: 13, marginLeft: 7 }, pressed: { opacity: 0.75 }, errorCard: { backgroundColor: colors.redDeep, marginTop: spacing.md }, errorText: { color: colors.text } });

...
export default function WorkOrdersScreen() {
  ...
  return <ScrollView contentContainerStyle={styles.root} showsVerticalScrollIndicator={false}>
    <View style={styles.header}><View><Text style={styles.eyebrow}>FIELD QUEUE</Text><Text style={styles.heading}>Work orders</Text></View><View style={styles.count}><Text style={styles.countValue}>{showArchive ? archivedRows.length : activeRows.length}</Text><Text style={styles.countLabel}>{showArchive ? 'ARCHIVED' : 'ACTIVE'}</Text></View></View>
    <View style={styles.queueTabs}>
      <Pressable onPress={() => setShowArchive(false)} style={[styles.queueTab, !showArchive && styles.queueTabActive]}><Text style={[styles.queueTabText, !showArchive && styles.queueTabTextActive]}>Active work</Text></Pressable>
      <Pressable onPress={() => setShowArchive(true)} style={[styles.queueTab, showArchive && styles.queueTabActive]}><Text style={[styles.queueTabText, showArchive && styles.queueTabTextActive]}>Archive ({archivedRows.length})</Text></Pressable>
    </View>
    ...
    <SectionHeader title={showArchive ? "Completed / filed work" : "Active work"} />
    ...
    </Card></Pressable>) : <EmptyState icon={showArchive ? "archive-outline" : "construct-outline"} title={showArchive ? "Archive is empty" : "Queue is clear"} message={showArchive ? 'Completed, cancelled, and closed work orders are filed here and kept out of the active queue.' : canCreate(membership) ? 'Use AI Walkthrough if your profile is authorized, or create a manual work order for an exception.' : 'New work orders will appear here when assigned.'} />}
  </ScrollView>;
}

const styles = StyleSheet.create({ root: { backgroundColor: colors.background, flexGrow: 1, padding: spacing.lg, paddingBottom: 110 }, header: { alignItems: 'center', flexDirection: 'row', justifyContent: 'space-between', marginBottom: spacing.md, paddingTop: spacing.sm }, queueTabs: { backgroundColor: colors.surface, borderColor: colors.border, borderRadius: 14, borderWidth: 1, flexDirection: 'row', gap: 4, marginBottom: spacing.lg, padding: 4 }, queueTab: { alignItems: 'center', borderRadius: 10, flex: 1, justifyContent: 'center', minHeight: 42, paddingHorizontal: spacing.sm }, queueTabActive: { backgroundColor: colors.tealDeep }, queueTabText: { color: colors.muted, fontSize: 13, fontWeight: '800' }, queueTabTextActive: { color: colors.teal }, eyebrow: { color: colors.teal, fontSize: 11, fontWeight: '800', letterSpacing: 1.2 }, heading: { color: colors.text, ...typography.title, marginTop: 4 }, count: { alignItems: 'flex-end' }, countValue: { color: colors.text, fontSize: 26, fontWeight: '800' }, countLabel: { color: colors.subtle, fontSize: 10, fontWeight: '800', letterSpacing: 1 }, aiCreate: { alignItems: 'center', backgroundColor: colors.tealDeep, borderColor: colors.teal, borderRadius: 16, borderWidth: 1, flexDirection: 'row', gap: spacing.md, marginBottom: spacing.md, padding: spacing.md }, aiIcon: { alignItems: 'center', backgroundColor: colors.background, borderRadius: 999, height: 48, justifyContent: 'center', width: 48 }, aiCopy: { flex: 1 }, aiEyebrow: { color: colors.teal, fontSize: 9, fontWeight: '900', letterSpacing: 1 }, aiTitle: { color: colors.text, fontSize: 16, fontWeight: '900', marginTop: 3 }, aiText: { color: colors.muted, fontSize: 11, lineHeight: 17, marginTop: 3 }, create: { alignItems: 'center', backgroundColor: colors.teal, borderRadius: 14, flexDirection: 'row', gap: spacing.sm, justifyContent: 'center', minHeight: 52, marginBottom: spacing.md }, createText: { color: colors.background, fontSize: 14, fontWeight: '900' }, search: { alignItems: 'center', backgroundColor: colors.surface, borderColor: colors.border, borderRadius: 14, borderWidth: 1, flexDirection: 'row', minHeight: 52, paddingHorizontal: spacing.md }, searchInput: { color: colors.text, flex: 1, fontSize: 15, marginLeft: spacing.sm }, orderCard: { marginBottom: spacing.sm }, card: { padding: spacing.md }, orderTop: { alignItems: 'center', flexDirection: 'row', justifyContent: 'space-between' }, status: { color: colors.muted, fontSize: 12, fontWeight: '700', textTransform: 'capitalize' }, orderTitle: { color: colors.text, fontSize: 17, fontWeight: '800', marginTop: spacing.md }, location: { color: colors.text, fontSize: 13, marginTop: spacing.sm }, department: { color: colors.teal, fontSize: 12, fontWeight: '800', marginTop: 5 }, metaLine: { alignItems: 'center', flexDirection: 'row', marginTop: spacing.md }, metaText: { color: colors.muted, fontSize: 13, marginLeft: 7 }, pressed: { opacity: 0.75 }, errorCard: { backgroundColor: colors.redDeep, marginTop: spacing.md }, errorText: { color: colors.text } });
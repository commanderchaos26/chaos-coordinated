...
const ARTWORK = require('../../assets/branding/chaos-intro-brand.jpg');

const absoluteFill = { position: 'absolute' as const, top: 0, right: 0, bottom: 0, left: 0 };

// The last pixel finishes at 1085 ms; hold only after the whole frame settles.
const ASSEMBLY_MS = 1100;
const HOLD_MS = 2000;
const EXIT_MS = 340;
...
export function BrandedIntro({
  ...
  const started = layoutReady && imageReady;
  ...
  useEffect(() => {
    ...
    const assemblyMs = reduceMotion ? 0 : ASSEMBLY_MS;
    ...
    scan.value = withTiming(1, {
      duration: Math.max(0, assemblyMs - 80),
    ...
    const completeTimer = setTimeout(onComplete, assemblyMs + HOLD_MS + exitMs);

    return () => {
      cancelAnimation(assemble);
      cancelAnimation(scan);
      cancelAnimation(exit);
      clearTimeout(exitTimer);
      clearTimeout(completeTimer);
    };
  }, [assemble, exit, onComplete, reduceMotion, scan, started]);
  ...
  return (
    ...
    >
      ...
      {!reduceMotion && <Animated.View pointerEvents="none" style={[styles.scanLine, scanStyle]} />}
    </Animated.View>
  );
}

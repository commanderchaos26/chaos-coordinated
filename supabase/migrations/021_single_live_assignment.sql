-- Enforce one live assignee per work order and make reassignment atomic.
-- Reassigning cancels any previous live assignment, preserves in-progress status
-- when the same assignment is merely rescheduled, and avoids duplicate offer notices.

create unique index if not exists one_live_assignment_per_work_order
  on public.assignments(work_order_id)
  where status in ('offered','accepted','active','paused','submitted');
...
begin
 ...
 if jsonb_array_length(v_conflicts)>0 and not p_override_availability then
   raise exception 'employee_unavailable';
 end if;
 if jsonb_array_length(v_conflicts)>0
    and p_override_availability
    and nullif(trim(p_override_reason),'') is null
